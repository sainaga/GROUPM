...
Route::post('/search_events', function(){

    // session(['my_locale' => Request::Input('locale')]);
    $in = Input::get ( 'search' );

    // return redirect()->back();
    session()->flash('success', 'Finish search by filling the options!');
     return redirect('/browse/events/'.$in);
});
Route::get('/browse/events/{ok}', 'ScreeningController@browse_events');
